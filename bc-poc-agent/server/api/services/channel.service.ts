...
import {
    ChannelEventHub, Peer, ProposalResponse, ChaincodeInvokeRequest,
    ChaincodeQueryRequest, ChannelInfo
} from 'fabric-client';
...
export class ChannelService {
    ...
    async GetChainInfo(peer: string, username: string, org: string) {
    ...
    async GetChannels(peer: string, username: string, org: string) {
        const target = this.buildTarget(peer, org);
        const channel = helper.getChannelForOrg(org);
        const client = helper.getClientForOrg(org);
        const user = await helper.getRegisteredUsers(username, org);
